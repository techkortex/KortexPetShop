...
export function Shop(){
  ...
  return <>
    <section className="shop-hero">
      ...
      <div className="shop-hero-art"><Image src="/images/kortex-products.png" alt="Seleção ilustrada de produtos da KortexPetShop" fill priority sizes="(max-width: 760px) 100vw, 50vw"/><span><Sparkles/> testado pela matilha</span></div>
    </section>
    ...
    <section className="catalog section-shell" aria-labelledby="catalog-title">
      ...
      {filtered.length===0?<div className="no-results"><Search/><h3>Nada por aqui ainda.</h3><p>Tente outro termo ou explore todas as categorias.</p><button className="button button-outline" onClick={()=>{setQuery('');setCategory('Todos')}}>Limpar filtros</button></div>:<div className="product-grid">{filtered.map((p)=><article className="product-card" key={p.id}>
        <div className={`product-art ${p.art}`}>{p.badge&&<span className={p.badge==='Novidade'?'new':''}>{p.badge}</span>}<button className="quick-add" onClick={()=>add(p.id)} aria-label={`Adicionar ${p.name} à sacola`}><Plus/></button></div>
